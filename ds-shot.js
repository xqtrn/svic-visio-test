  ...
  for (const [eng, name, vp] of [[chromium, 'desktop', { width: 1440, height: 1000 }], [webkit, 'ioswk', { width: 390, height: 844 }]]) {
    ...
    const probe = await pg.evaluate(() => {
      ...
      return { sections: secs.length, navOk: nav.every(h => secs.includes(h)), groups,
               psSubs: ps.querySelectorAll('.bb-sub').length, psPreviews: ps.querySelectorAll('[data-code]').length, unlockGuide: !!document.getElementById('motion-unlock'), entryGuide: !!document.getElementById('motion-entry') };
    });
